...
namespace LearningBasics._6.DynamicProgramming
{
    ...
    {//Count the number of unique paths from the top left to the bottom right. You are only allowed to move down or to the right.
     // Brute Force - Time: O(2 ^ (n + m)), Space: O(n + m)
        int[][] blockingGrid = { [0, 1], [0, 0], };
        public TwoDimensionalDynamicProgramming()
        {

            //UniquePaths(3, 6);
            UniquePathsWithObstacles(blockingGrid);
        }
        ...
        {
            //used to store the previous values and we dont need to have a full metrics
            ...
            for (int i = rows - 1; i >= 0; i--)
            {
                ...
                // then we push left, starting, cols - 2 is because we adding 1 already and we dont need to look at it again
        ...
        public int UniquePathsWithObstacles(int[][] obstacleGrid)
        {
            ...
            for (int i = rowCount - 1; i >= 0; i--)
            {
                ...
                if (obstacleGrid[i][colLength - 1] == 0)
                {
                    current[colLength - 1] = 1;
                }
                ...
                for (int j = colLength - 2; j >= 0; j--)
                {
                    ...
                    {
                        //dp[i][j - 1] + dp[i - 1][j]
                        current[j] = current[j + 1] + previousRow[j];
                    }
                }
                previousRow = current;
            }
